...
Reused unchanged from the Assignment 3 SORT experiment (Bewley et al., ICIP
2016, https://arxiv.org/abs/1602.00763) -- see reuse_audit.md.

Decides which predicted track goes with which new detection: builds an IoU
cost matrix between predicted boxes and detections, then solves it with the
Hungarian algorithm for an optimal one-to-one matching (rather than greedily
grabbing the first good-looking match). ByteTrack's two-stage BYTE association
calls this same function twice per frame -- once for the high-confidence
first association, once for the low-confidence second association -- with the
second call restricted to detections and tracks left unmatched by the first.
"""
